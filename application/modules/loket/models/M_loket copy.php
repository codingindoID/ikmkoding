...
class M_loket extends CI_Model
{
	...
	function detil_loket($data, $tahun, $bulan, $totalResponden)
	{
		$hasil 	= [];
		$no 	= 1;
		if ($data) {
			foreach ($data as $dat) {
				$responden_loket = $this->_jumlahResponden($dat->id_loket, $tahun, $bulan);
				$hasil[$no++] = [
					'id_loket'				=> $dat->id_loket,
					'jenis_layanan'			=> $dat->nama_loket,
					'jumlah_responden'		=> count($responden_loket),
					'total_semua'			=> $totalResponden,
					'persen'				=> ($totalResponden > 0) ? (count($responden_loket) / $totalResponden) * 100 : 0,
					'kepuasan'				=> 12,
	...
	{
		if ($bulan == 'setahun') {
			$query2 = "SELECT * FROM tb_hasil JOIN tb_detil_responden ON tb_detil_responden.id_responden = tb_hasil.id_responden  WHERE YEAR ( tb_hasil.created_date ) = '$tahun' AND loket = '$id_loket' AND published = '2' GROUP BY tb_hasil.id_responden";
		} else {
			$query2 = "SELECT * FROM tb_hasil JOIN tb_detil_responden ON tb_detil_responden.id_responden = tb_hasil.id_responden  WHERE YEAR ( tb_hasil.created_date ) = '$tahun' and MONTH(tb_hasil.created_date) = '$bulan' AND loket = '$id_loket' AND published = '2' GROUP BY tb_hasil.id_responden";
		}
